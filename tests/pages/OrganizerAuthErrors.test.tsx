...
describe('organizer auth error handling', () => {
  ...
  it('shows permission denied instead of event settings controls', async () => {
    ...
    render(
      <EventSettingsPage params={resolvedParams({ eventId: 'event-boston' })} />
    );
    ...
    expect(
      screen.queryByText(/no staff has been assigned/i)
    ).not.toBeInTheDocument();
  });
  ...
  it('does not render chapter settings after public chapter fetch when protected details are forbidden', async () => {
    ...
    render(
      <ChapterSettingsPage params={resolvedParams({ chapterSlug: 'boston' })} />
    );
    ...
    expect(
      screen.queryByRole('button', { name: /invite admin/i })
    ).not.toBeInTheDocument();
  });
});
