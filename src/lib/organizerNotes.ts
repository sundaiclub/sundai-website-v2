...
export function hasSharedOrganizerNoteEvent(
  relevance: Pick<OrganizerNoteRelevance, 'assignedEventStaff' | 'targetEventIds'>
): boolean {
...
export async function getOrganizerNoteRelevance(
  ...
  const [actor, chapterAdminMemberships, assignedStaff, targetMemberships, targetRegistrations] =
    await Promise.all([
...
export async function getCurrentOrganizerNoteForActor(
  ...
  const relevance = await getOrganizerNoteRelevance(actorId, targetHackerId, db)
