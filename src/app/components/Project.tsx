...
export function ProjectCard({
  ...
  const teamMembersBase = [
    {
      ...
      avatarUrl: project.launchLead.avatar?.url || null,
    },
    ...project.participants.map(p => ({
      id: p.hacker.id,
      name: p.hacker.name,
      avatarUrl: p.hacker.avatar?.url || null,
    })),
  ];
  ...
  const remainingMembers = Math.max(
    0,
    teamMembers.length - visibleMembers.length
  );
  ...
  const previewMinHeightClass =
    variant === 'trending' ? 'min-h-[28px]' : 'min-h-[32px]';
  ...
  return (
    ...
    >
        ...
        <div className="absolute top-3 right-3 z-10">
          ...
          >
            <div className="flex items-center space-x-2">
              <svg
                className={`${project.likes.some(like => like.hackerId === userInfo?.id) ? 'fill-current' : ''} w-5 h-5`}
                viewBox="0 0 24 24"
              >
      ...
      <div className={`${cardPaddingClass} flex-1 flex flex-col`}>
            ...
            {show_team && (
              <div className="flex items-center">
                ...
                {teamMembers.length > 5 && (
                  <div
                    className={`-ml-2 w-6 h-6 rounded-full flex items-center justify-center text-xs ${isDarkMode ? 'bg-gray-700 text-gray-200' : 'bg-gray-100 text-gray-700'} border-2 ${isDarkMode ? 'border-gray-800' : 'border-white'}`}
                  >
                    +{teamMembers.length - 5}
                  </div>
                )}
        ...
        <div
          className={`flex flex-wrap gap-2 mt-auto ${tagsBottomMarginClass} min-h-[24px]`}
        >
          ...
          {(variant === 'compact' || variant === 'trending') &&
            project.techTags.length > maxTechTags && (
              <span
                className={`inline-flex items-center h-6 px-2 rounded-full text-xs font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}
              >
                +{project.techTags.length - maxTechTags}
              </span>
            )}
        ...
        {variant !== 'trending' && (
          <div
            className={`pt-4 border-t border-gray-200 flex items-center justify-between`}
          >
...
export default function ProjectGrid({
  ...
  return (
    <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-8 py-6 sm:py-12">
      ...
      <div
        className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 ${variant === 'compact' ? 'gap-3 sm:gap-4' : 'gap-4 sm:gap-6'}`}
      >
      ...
      {enablePagination && hasMoreProjects && !requiresFullDataset && (
        <div
          ref={loadMoreRef}
          className="flex min-h-8 justify-center py-8"
          aria-hidden="true"
        >
          {isFetchingMore && (
            <div
